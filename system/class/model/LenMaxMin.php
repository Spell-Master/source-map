...
class LenMaxMin {
    ...
    private function checkLen() {
        ...
        if (!isset($relStr) || empty($relStr)) {
            $this->answer = "O campo \"{$this->chekType}\" está vazio";
            return(true);
        } else if (strlen($relStr) < $this->minLen) {
            $this->answer = "O campo \"{$this->chekType}\" possui menos de {$this->minLen} caracteres";
            return(true);
        } else if (strlen($relStr) > $this->maxLen) {
            $this->answer = "O campo \"{$this->chekType}\" possui mais de {$this->maxLen} caracteres";
            return(true);
        }
    }
}
