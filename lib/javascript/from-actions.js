...
function searchPage(file, len) {
    ...
    try {
        if (!$search) {
            ...
            smc.modal.saveform('search-page', file + '.php', 'Pesquisar', true);
        }
    } catch (e) {
...
function saveApp(len, type) {
    ...
    try {
        if (type !== 'new' && type !== 'edit') {
            ...
            smc.modal.saveform(type + '-app', 'app.php', (type == 'new' ? 'Nova' : 'Editar') + ' Página', true);
        }
    } catch (e) {
        smc.modal.error(e, true);
    }
