...
(function () {
    ...
    smStf = (function () {
        ...
        var pageAction = {
            cancel: function () {
                var $action = document.getElementById('page-action'),
                        $preview = document.getElementById('page-preview');
                if ($action.innerHTML.length >= 1) {
                    $action.innerHTML = null;
                }
                if ($preview.innerHTML.length >= 1) {
        ...
        var doc = {
            openNew: function (file) {
                if (file && (file == 'sector' || file == 'categ')) {
                    smTools.scroll.top();
                    document.getElementById('paginator').classList.add('hide');
            ...
            openEdit: function (file, hash) {
                if ((file && hash) && (file == 'sector' || file == 'categ')) {
                    smTools.scroll.top();
                    document.getElementById('paginator').classList.add('hide');
