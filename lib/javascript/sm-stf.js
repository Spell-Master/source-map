...
(function () {
    ...
    smStf = (function () {
        ...
        var doc = {
            add: function (file, tgtOn) {
                smTools.scroll.top();
                document.getElementById('paginator').classList.add('hide');
                smTools.ajax.send('page-action', 'modules/admin/doc/' + file + '-new.php' + (tgtOn ? '?target=' + tgtOn : ''), false);
            }, edit: function (file, hash) {
                smTools.scroll.top();
                document.getElementById('paginator').classList.add('hide');
                smTools.ajax.send('page-action', 'modules/admin/doc/' + file + '-edit.php?hash=' + hash, false);
            }, del: function (file, hash) {
                smCore.modal.dangerAction('del-' + file + '-' + hash, 'admin/doc/' + file + '-del_v.php', 'Apagar');
