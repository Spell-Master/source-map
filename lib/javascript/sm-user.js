    ...
    smUser = (function () {
        ...
        var uploadAtt = function (params) {
            ...
            if (params.model == 'image') {
                $clearDiv = document.getElementById('not-img');
                $divAtt = document.getElementById('img-att');
                $imgAtt = document.createElement('img');
                $imgAtt.src = 'uploads/' + params.hash + '/' + params.img;
            } else {
                $clearDiv = document.getElementById('not-file');
                $divAtt = document.getElementById('file-att');
                $imgAtt = document.createElement('div');
                $imgAtt.className = 'box-xy-50 bg-dark radius-circle margin-auto';
                $imgAtt.innerHTML = '<i class="icon-file-zip2"></i>';
            }

            if (smTools.check.isReady($clearDiv)) {
                if ($clearDiv.parentNode) {
                    $clearDiv.parentNode.removeChild($clearDiv);
                }
            }

            $userAtt = document.createElement('div');
            $userAtt.classList.add('user-attachment');
            $userAtt.setAttribute('data-att', params.id);
            $userAtt.appendChild($imgAtt);

            $pAtt = document.createElement('p');
            $pAtt.innerHTML = params.size + ' ' + params.date;
            $userAtt.appendChild($pAtt);

            $inputAtt = document.createElement('input');
            $inputAtt.type = 'checkbox';
            $inputAtt.name = 'delfile[]';
            $inputAtt.value = params.name;
            $inputAtt.id = 'delfile-' + params.idx;
            $inputAtt.classList.add('checkbox');
            $userAtt.appendChild($inputAtt);

            $labelAtt = document.createElement('label');
            $labelAtt.setAttribute('for', 'delfile-' + params.idx);
            $labelAtt.innerHTML = '<span class="italic"> Apagar</span>';
            $userAtt.appendChild($labelAtt);

            $divAtt.appendChild($userAtt);
            smTools.cart.add($inputAtt);
            smTools.tab.open((params.model == 'image' ? 1 : 2));
        ...
        var delAttach = function () {
            smCore.modal.dangerAction('delatt', 'user/del-att_v.php', 'Apagar Anexos');
        };
