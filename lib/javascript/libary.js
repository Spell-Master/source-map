...
var ItemOpen = function () {
    ...
    function openItens(e) {
        if (e.which === 1) {
            $click = e.target;
            $itemClick = (smlib.isReady($click.dataset.open) ? $click.dataset.open : false);
            NodeParents();
            $fix = null;
    ...
    function NodeParents() {
        var $parents = [];
        while ($click.parentNode != null && $click.parentNode != document.documentElement) {
            $parents.push($click.parentNode);
